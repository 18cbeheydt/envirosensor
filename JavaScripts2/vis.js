...
function getRowsBy(value, attr, data){
...
/* Sum up all attribute values of the given rows */
function getSumOf(attr, rows){
...
/* Draw the title of the bar chart */
...
/* Draw switches to show different datasets */
...
/* Called when the user push a switch to change the dataset to show */
function switchStackBarChart (index){
	...
	/* Keep track of information that which switch is now active */
	ACTIVE_IDX = index;
...
function createVis(error, geodata, sheet1, sheet2){
	...
	/* set up an svg to show the title of the chart */
	var h = 60,
		w = 320;
	...
	/* Used to handle country aliases */
	var country_alias = {
	...
	/* Draw countries, and connect events to each */
	g.selectAll("path").data(geodata.features).enter()
		...
		.on("mouseleave", function (d) {
			d3.select("#map_tooltip").style("display", "none");
			IS_SEA = true;
		});
...
/* Called when the user clickes a country with no data */
